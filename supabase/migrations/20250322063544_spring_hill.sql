...
CREATE OR REPLACE FUNCTION increment(row_id UUID, table TEXT, column_name TEXT)
RETURNS INTEGER AS $$
DECLARE
  current_value INTEGER;
  new_value INTEGER;
BEGIN
  EXECUTE format('SELECT %I FROM %I WHERE id = $1', column_name, table)
  INTO current_value
  USING row_id;
...
CREATE OR REPLACE FUNCTION add_to_column(row_id UUID, table TEXT, column_name TEXT, value_to_add INTEGER)
RETURNS INTEGER AS $$
DECLARE
  current_value INTEGER;
  new_value INTEGER;
